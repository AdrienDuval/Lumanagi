...
const alchemyApiKey = "1ErT-b6cMMBC6D2qmzM8EuNeoaQZl8XZ";
if (!alchemyApiKey) {
  console.error(
...
// Use alchemyUrl in your connection logic
...
io.on("connection", (socket) => {
  ...
  socket.on("joinRoom", async (data) => {
    ...
    const [ticketMessage, error] = await promisePool.query(
      `select * from ticket_message where ticket_id =${ticket_id}`
    );
  ...
  socket.on("chatMessage", async (newChatdata) => {
    ...
    const [insertData, error1] = await promisePool.query(
      `insert into ticket_message SET ticket_id='${ticket_id}',sender='${sender}',receiver='${receiver}',message='${message}'`
    );
